...
Copy everything below the `---` line and paste it into Claude. Replace the `[REPLACE THIS WITH THE ACTUAL TASK]` block with the specific feature or bug you want help with.
...
- Backend: Lovable Cloud (Supabase-managed) — in user-facing replies, never say "Supabase"; say Lovable Cloud / backend / database / auth / storage / functions
- Auth: Lovable Cloud auth with Google OAuth; roles are stored in a separate `public.user_roles` table, never on the `users` or `profile` tables
- UI: shadcn/ui, dark-first industrial-EPC palette, semantic-only tokens (no raw hex, rgb, or arbitrary color utilities)
