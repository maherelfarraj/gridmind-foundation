...
import { AGE_BAND_TEXT, bandForAge, formatCurrency, type AgeBand } from "@/lib/risks.rules";
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
      <Kpi icon={<ClipboardList size={16} aria-hidden />} label="Open risks">
        <span className="text-2xl font-semibold text-foreground">{openCount}</span>
      </Kpi>
      ...
      <Kpi icon={<PiggyBank size={16} aria-hidden />} label="Contingency exposure">
        {contingency.primary ? (
      ...
      <Kpi icon={<Timer size={16} aria-hidden />} label="Register age">
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <span className={cn("cursor-help text-2xl font-semibold", AGE_BAND_TEXT[band])}>
