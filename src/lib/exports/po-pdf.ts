...
export async function buildPoPdfBytes(input: PoPdfInput): Promise<Uint8Array> {
  ...
  // Footer
  const footerText = sanitize(branding.footerText || `Generated by GridMind EPC · ${company.name}`);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (po.issued_at) {
      doc.text(`Issued ${fmtDate(po.issued_at)}`, pageW / 2, pageH - 24, { align: "center" });
    }
  }
