...
export async function buildWeeklyReportPdfBytes(
  input: WeeklyReportDTO,
): Promise<Uint8Array> {
  ...
  doc.setFontSize(18);
  doc.text(
    sanitize(input.company.legalName || input.company.name),
    margin + (logo ? 66 : 0),
    44,
  );
  ...
  doc.text(
    sanitize(
      `Weekly Construction Report — ${input.project.name} · ${input.isoWeekLabel}`,
    ),
    margin + (logo ? 66 : 0),
    64,
  );
  ...
  const rightMeta = [
    ...
    input.project.capacityMw != null
      ? `${fmtNum(input.project.capacityMw, 1)} MW`
      : null,
  ...
  // ---- Lookahead --------------------------------------------------------
  if (
    input.lookahead.topWeatherImpacts.length > 0 ||
    input.lookahead.plannedAreas.length > 0
  ) {
    cursorY = ensureRoom(doc, cursorY, 140, margin, pageH);
  ...
  const footerText = sanitize(
    input.branding.footerText ||
      `Generated by GridMind EPC · ${input.company.name}`,
  );
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      sanitize(`${input.project.name} · ${input.isoWeekLabel}`),
      pageW / 2,
      pageH - 24,
      { align: "center" },
    );
...
async function drawPhotoStrip(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (items[i].caption) {
      ...
      doc.text(
        sanitize(items[i].caption).slice(0, 60),
        cx,
        cy + cellH + 12,
        { maxWidth: cellW },
      );
...
export function weeklyReportFilename(
  projectName: string,
  weekLabel: string,
): string {
