...
import { OPPORTUNITY_STAGES, STAGE_PROBABILITY, type OpportunityStage } from "@/lib/crm.functions";
...
export const deleteContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if ((row as any).opportunity_id) {
      await auditOpportunity(context, "contact.deleted", (row as any).opportunity_id, {
        contact_id: data.id,
...
export const deleteTenderEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("tender_events").delete().eq("id", data.id);
    if (error) throw error;

    if ((row as any).opportunity_id) {
      await auditOpportunity(context, "tender_event.deleted", (row as any).opportunity_id, {
        tender_event_id: data.id,
...
export const convertOpportunityToIntake = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ intake_id: string; alreadyConverted: boolean }> => {
    requireSupabaseAuth(context);
    await assertCrmWriter(context);
...
export const buildKickoffPack = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ path: string }> => {
    ...
    // Related data
    const [{ data: contacts }, { data: proposals }, { data: tenders }] = await Promise.all([
      context.supabase
        .from("contacts")
    ...
    const acceptedProposal =
      (proposals ?? []).find((p: any) => ["accepted", "sent"].includes(p.status)) ??
      (proposals ?? [])[0] ??
      null;
