...
| `LOVABLE_API_KEY`               | Lovable AI Gateway + all outbound email (auth, notifications, scheduled reports) | yes | Managed (auto-provisioned) | Do not set manually. Rotate via the Lovable AI Gateway tool, not the secrets tool. Missing → dispatcher records `skipped` with `email_not_configured`. |
...
> **EmailJS retired (P-270).** All `EMAILJS_*` secrets were removed. Outbound
> mail now leaves through the managed sender `notify.gridmindepc.com` using
> `LOVABLE_API_KEY`; templates live in `src/lib/email-templates/` and the event
> map in `src/lib/email/registry.ts`. No external mail provider keys are needed.
