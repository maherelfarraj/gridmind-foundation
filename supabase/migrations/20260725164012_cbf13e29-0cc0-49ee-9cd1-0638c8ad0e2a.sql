drop policy if exists project_phase_gates_admin on public.project_phase_gates;
create policy project_phase_gates_admin on public.project_phase_gates for all to authenticated
  ...
  if coalesce(v_exposure->>'photos','false') = 'true'
    ...
    v_out := v_out || jsonb_build_object(
      ...
      coalesce((
        ...
        from (
          select sp.id, sp.file_path, sp.caption, sp.taken_at, sp.discipline
