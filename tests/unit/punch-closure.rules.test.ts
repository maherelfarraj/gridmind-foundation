...
describe("commissioning-punch rules", () => {
  ...
  it("category A + utility witness requires all three", () => {
    expect(requiredParties("A", true)).toEqual([
      "contractor",
      "client",
      "utility",
    ]);
  ...
  it("missingParties reports what is missing", () => {
    expect(missingParties(["contractor", "client"], ["contractor"])).toEqual([
      "client",
    ]);
  ...
  it("A + utility waits for all three", () => {
    ...
    expect(canCloseNow("A", true, ["contractor", "client", "utility"])).toBe(
      true,
    );
  });
  ...
  it("duplicate parties don't change closure decision", () => {
    ...
    expect(canCloseNow("A", false, ["contractor", "contractor", "client"])).toBe(
      true,
    );
  });
});
